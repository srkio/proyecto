...
namespace CapaPresentacion
{
    public partial class Form1 : Form
    {
        ...
        private void seleccionarUnAtracador()
        {
            try
            {
                int row = listadoAtracador.CurrentCell.RowIndex;
                ...
                char gen = (char)listadoAtracador.Rows[row].Cells[5].Value;
                if (gen == 'm') {
                ...
                listadoDeBandas.SelectedIndex = ((int)listadoAtracador.Rows[row].Cells[7].Value) - 1;
            }
            catch (Exception)
            {
                throw;
